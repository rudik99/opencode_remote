...
Quick reference checklist for web application performance. Use alongside the `performance-optimizer` subagent.
...
Use the `performance-optimizer` subagent for decisions about layers, invalidation strategy, and what must never be cached. This section covers the read/write patterns and the checklist.
